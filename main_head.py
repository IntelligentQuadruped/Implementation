...
def filterOutlier(data_list,z_score_threshold):
	"""
	...
	data_out = data[np.where(np.abs(z_scores)>z_score_threshold)]
...
def sendDirection(n,r,fracx):
	...
	frac_prime = None# when head was used to find a gap
	if gap_direction != 0:
